...
public class GaAnt {
    ...
    public void chooseNextCity() {
        while (allowed.size() > 0) {
            double[] prob = Arrays.copyOf(GaCity.getProb(currentCity), cityNum);
...
//        roadLength += City.getDistance(currentCity, tabu.get(0));
    ...
    public double getRoadLength() {
        if (roadLength<=0 ){
            Integer[] pathArr = new Integer[cityNum];
            path.toArray(pathArr);
            roadLength = DistanceUtil.getDistance(pathArr);
        }
        return roadLength;
    }
...
//            p += tabu.get(i) + ";";
//        }
        return Arrays.toString(getPath().toArray());
    }
